...
function getNewEvents(arrayEventos, currentDate) {
...
getNewEvents(arrayDatos, currentDate);
...
generateCards(newEvents);

function searchEvents() {
    const searchbox = document.getElementById("searchbox");
    const query = searchbox.value.toLowerCase().trim();
    if (query.length > 0) {
        filteredEvents = newEvents.filter(evento =>
...
function filterEvents() {
    ...
    if (selectedCategories.length > 0) {
      filteredEvents = newEvents.filter(
